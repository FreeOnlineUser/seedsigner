...
class DPI28(BaseDisplayDriver):
    ...
    def _init_framebuffer(self):
        ...
        try:
            ...
            logger.info(f"Using: {'numpy' if HAS_NUMPY else 'Python'} conversion")
    ...
    def show_image(self, image: Image.Image, x_start: int = 0, y_start: int = 0):
        ...
        self._write_to_fb(self.compose(image))
...
@dataclass
class DPI28Emulator(DPI28):
    """
    Headless stand-in for the DPI28 driver: identical 480x640 composition
    (2x-scaled UI + touch bar) but frames go to a PNG file / callback instead
    of /dev/fb0. Used by the desktop emulator and the screenshot generator so
    layout work needs no hardware.
    """

    # Every frame is written here (overwritten each time) when set
    frame_path: str = None
    # When set, every frame is also saved numbered under this dir
    frames_dir: str = None

    def __post_init__(self):
        # Deliberately skip DPI28.__post_init__'s framebuffer/tty setup
        self.fb = None
        self.fb_file = None
        self.tty_fd = None
        self.frame_count = 0
    ...
    def show_image(self, image: Image.Image, x_start: int = 0, y_start: int = 0):
        ...
        self.frame_count += 1
        if self.frame_path:
            display.save(self.frame_path)
        if self.frames_dir:
            os.makedirs(self.frames_dir, exist_ok=True)
            display.save(os.path.join(self.frames_dir, f"frame_{self.frame_count:05d}.png"))
        if self.on_frame:
