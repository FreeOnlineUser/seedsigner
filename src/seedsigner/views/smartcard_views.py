...
# These helpers carry their own internal pysatochip import guards, so they are safe
# to import unconditionally (and the views' runtime references need them bound even
# when pysatochip is absent, e.g. in the emulator/test environment).
...
# format_sw_error is our own helper (upstream 3rdIteration code imported it from a
# stale "pysatochip.satochip_protocol_helper" path that exists in no pysatochip
# release; every helper module already uses iso7816).
from seedsigner.helpers.iso7816 import format_sw_error

try:
    # pysatochip only exists on-device; guarding the imports keeps this module
    # importable in the test/dev environment. Note UnexpectedSW12Error is defined
    # in CardConnector (upstream's "pysatochip.exception" import path is stale and
    # would silently unbind this whole block on-device).
    ...
    class UnexpectedSW12Error(Exception):
        # Stand-in so except-clauses stay valid when pysatochip is absent
        sw1 = None
        sw2 = None
