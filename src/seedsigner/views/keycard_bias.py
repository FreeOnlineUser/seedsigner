"""Signing bias test for Keycard backends."""
import csv
import math
import os
import random
...
try:
    # Only present on-device (depends on pysatochip)
...
class ToolsKeycardBiasCheckView(View):
    """Run multiple signatures to look for ECDSA bias (Keycard backend)."""
    ...
    def run(self):
        ...
        connector = seedkeeper_utils.init_satochip(self, init_card_filter=["satochip"], backend_preference="keycard")
        if not connector:
            return Destination(BackStackView)

        # Keycard operations are slower (~1.0s each), so use a higher timeout
        timeout = 3.0
        ...
        csv_path = MicroSD.get_microsd_dir() / f"keycard_bias_{transport}.csv"
        txt_path = MicroSD.get_microsd_dir() / f"keycard_bias_{transport}.txt"
        try:
        ...
        logger.info("Keycard bias test results:\n%s", console_text)
        print(console_text)
